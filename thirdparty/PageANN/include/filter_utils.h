    ...
    std::vector<label_set> point_ids_to_labels, label_set all_labels)
{
    ...
    pageann::MemoryMapper input_data(input_data_path);
...
inline std::vector<uint32_t> loadTags(const std::string &tags_file, const std::string &base_file)
{
    ...
    if (tags_enabled)
    {
        ...
        pageann::load_bin<std::uint32_t>(tags_file, tag_data, tag_file_npts, tag_file_ndims);
        if (tag_file_ndims != 1)
        {
            pageann::cerr << "tags file error" << std::endl;
            throw pageann::ANNException("tag file error", -1, __FUNCSIG__, __FILE__, __LINE__);
        }
        // // check if the point count match
        size_t base_file_npts, base_file_ndims;
        pageann::get_bin_metadata(base_file, base_file_npts, base_file_ndims);
        if (base_file_npts != tag_file_npts)
        {
            ...
            pageann::cerr << "point num in tags file mismatch" << std::endl;
            throw pageann::ANNException("point num in tags file mismatch", -1, __FUNCSIG__, __FILE__, __LINE__);
        }
