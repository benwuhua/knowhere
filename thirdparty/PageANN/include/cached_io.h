  ...
  public:
    ...
    void open(const std::string &filename, uint64_t cacheSize)
    {
        ...
        try
        {
            ...
            pageann::cout << "Opened: " << filename.c_str() << ", size: " << fsize << ", cache_size: " << cacheSize
                          << std::endl;
        }
        catch (std::system_error &e)
        {
            throw pageann::FileException(filename, e, __FUNCSIG__, __FILE__, __LINE__);
        }
    }
  ...
  public:
    cached_ofstream(const std::string &filename, uint64_t cache_size) : cache_size(cache_size), cur_off(0)
    {
        ...
        try
        {
            ...
            pageann::cout << "Opened: " << filename.c_str() << ", cache_size: " << cache_size << std::endl;
        }
        catch (std::system_error &e)
        {
            throw pageann::FileException(filename, e, __FUNCSIG__, __FILE__, __LINE__);
    ...
    void close()
    {
        ...
        pageann::cout << "Finished writing " << fsize << "B" << std::endl;
    }
