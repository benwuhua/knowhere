...
inline bool file_exists_impl(const std::string &name, bool dirCheck = false)
{
    ...
    if (val != 0)
    {
        ...
        {
        case EINVAL:
            pageann::cout << "Invalid argument passed to stat()" << std::endl;
            break;
        case ENOENT:
            // file is not existing, not an issue, so we won't cout anything.
            break;
        default:
            pageann::cout << "Unexpected error in stat():" << errno << std::endl;
            break;
        }
        return false;
    }
...
inline void open_file_to_write(std::ofstream &writer, const std::string &filename)
{
    ...
    if (writer.fail())
    {
        ...
        pageann::cerr << message << std::endl;
        throw pageann::ANNException(message, -1);
    }
}
...
inline size_t get_file_size(const std::string &fname)
{
    ...
    {
        pageann::cerr << "Could not open file: " << fname << std::endl;
        return 0;
    }
}
...
inline int delete_file(const std::string &fileName)
{
    if (file_exists(fileName))
    {
        ...
        if (rc != 0)
        {
            pageann::cerr << "Could not delete file: " << fileName
...
{
    pageann::cerr << error_stream.str() << std::endl;
    throw pageann::ANNException(error_stream.str(), -1, __FUNCSIG__, __FILE__, __LINE__);
}
...
#else
    pageann::cerr << "No aligned realloc on GCC. Must malloc and mem_align, "
                     "left it out for now."
                  << std::endl;
#endif
    if (*ptr == nullptr)
...
inline void get_bin_metadata(MemoryMappedFiles &files, const std::string &bin_file, size_t &nrows, size_t &ncols,
                             size_t offset = 0)
{
    pageann::cout << "Getting metadata for file: " << bin_file << std::endl;
...
inline void load_bin(MemoryMappedFiles &files, const std::string &bin_file, T *&data, size_t &npts, size_t &dim,
                     size_t offset = 0)
{
    pageann::cout << "Reading bin file " << bin_file.c_str() << " at offset: " << offset << "..." << std::endl;
    auto fc = files.getContent(bin_file);
...
{
    pageann::cout << "Reading bin file " << bin_file.c_str() << " ..." << std::endl;
    std::ifstream reader;
    reader.exceptions(std::ifstream::failbit | std::ifstream::badbit);

    try
    {
        pageann::cout << "Opening bin file " << bin_file.c_str() << "... " << std::endl;
...
inline void load_truthset(const std::string &bin_file, uint32_t *&ids, float *&dists, size_t &npts, size_t &dim)
{
    ...
    pageann::cout << "Reading truthset file " << bin_file.c_str() << " ..." << std::endl;
    ...
    pageann::cout << "Metadata: #pts = " << npts << ", #dims = " << dim << "... " << std::endl;
                                ...
                                uint64_t &gt_num)
{
    size_t read_blk_size = 64 * 1024 * 1024;
    cached_ifstream reader(bin_file, read_blk_size);
    pageann::cout << "Reading truthset file " << bin_file.c_str() << "... " << std::flush;
    size_t actual_file_size = reader.get_file_size();
    ...
    pageann::cout << "Metadata: #pts = " << gt_num << ", #total_results = " << total_res << "..." << std::endl;
...
inline void open_file_to_write(std::ofstream &writer, const std::string &filename)
{
    ...
    if (writer.fail())
    {
        ...
        pageann::cerr << error_message << std::endl;
        throw pageann::ANNException(error_message, -1);
    }
}
...
inline size_t save_bin(const std::string &filename, T *data, size_t npts, size_t ndims, size_t offset = 0)
{
    ...
    pageann::cout << "Writing bin: " << filename.c_str() << std::endl;
    writer.seekp(offset, writer.beg);
    int npts_i32 = (int)npts, ndims_i32 = (int)ndims;
    size_t bytes_written = npts * ndims * sizeof(T) + 2 * sizeof(uint32_t);
    writer.write((char *)&npts_i32, sizeof(int));
    writer.write((char *)&ndims_i32, sizeof(int));
    pageann::cout << "bin: #pts = " << npts << ", #dims = " << ndims << ", size = " << bytes_written << "B"
                  << std::endl;

    writer.write((char *)data, npts * ndims * sizeof(T));
    writer.close();
    pageann::cout << "Finished writing bin." << std::endl;
                                  ...
                                  size_t &dim, size_t &rounded_dim)
{
    ...
    if (actual_file_size != expected_actual_file_size)
    {
        ...
        stream << "Error. File size mismatch. Actual size is " << actual_file_size << " while expected size is  "
               ...
               << std::endl;
        pageann::cout << stream.str() << std::endl;
        throw pageann::ANNException(stream.str(), -1, __FUNCSIG__, __FILE__, __LINE__);
    }
    rounded_dim = ROUND_UP(dim, 8);
    pageann::cout << "Metadata: #pts = " << npts << ", #dims = " << dim << ", aligned_dim = " << rounded_dim << "... "
                  << std::flush;
    size_t allocSize = npts * rounded_dim * sizeof(T);
    pageann::cout << "allocating aligned memory of " << allocSize << " bytes... " << std::flush;
    alloc_aligned(((void **)&data), allocSize, 8 * sizeof(T));
    pageann::cout << "done. Copying data to mem_aligned buffer..." << std::flush;
                             ...
                             size_t &rounded_dim)
{
    try
    {
        pageann::cout << "Opening bin file " << bin_file << " ..." << std::flush;
...
inline void load_aligned_bin(const std::string &bin_file, T *&data, size_t &npts, size_t &dim, size_t &rounded_dim)
{
    ...
    try
    {
        pageann::cout << "Reading (with alignment) bin file " << bin_file << " ..." << std::flush;
        reader.open(bin_file, std::ios::binary | std::ios::ate);
                                        ...
                                        const size_t &rounded_dim, size_t offset = 0)
{
    if (data == nullptr)
    {
        pageann::cerr << "Memory was not allocated for " << data << " before calling the load function. Exiting..."
                      << std::endl;
        throw pageann::ANNException("Null pointer passed to copy_aligned_data_from_file function", -1, __FUNCSIG__,
                                    __FILE__, __LINE__);
    }
    std::ifstream reader;
...
{
    if (!in.is_open())
        throw pageann::ANNException("Index file size check called on unopened file stream", -1, __FUNCSIG__, __FILE__,
                                    __LINE__);
    in.seekg(0, in.end);
    ...
    if (actual_file_size != expected_file_size)
    {
        pageann::cerr << "Index file size error. Expected size (metadata): " << expected_file_size
...
inline std::vector<std::string> read_file_to_vector_of_strings(const std::string &filename, bool unique = false)
{
    ...
    {
        throw pageann::ANNException(std::string("Failed to open file. filename can not be blank"), -1);
    }
    return result;
}
        ...
        for (const auto &path : paths_to_clean)
        {
            for (const auto &suffix : path_suffixes)
            {
                ...
                if (std::remove(curr_path_to_clean.c_str()) != 0)
                    pageann::cout << "Warning: Unable to remove file :" << curr_path_to_clean << std::endl;
            }
        }
        pageann::cout << "Cleaned all artifacts" << std::endl;
    }
    catch (const std::exception &e)
    {
        pageann::cout << "Warning: Unable to clean all artifacts " << e.what() << std::endl;
    }
}
