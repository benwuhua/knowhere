...
void gen_random_slice(const std::string base_file, const std::string output_prefix, double sampling_rate)
{
    ...
    pageann::cout << "Loading base " << base_file << ". #points: " << npts_u32 << ". #dim: " << nd_u32 << "."
                  << std::endl;
    ...
    pageann::cout << "Wrote " << num_sampled_pts_u32 << " points to sample file: " << output_prefix + "_data.bin"
                  << std::endl;
}
                             ...
                             const size_t k_base, std::string prefix_path)
{
    ...
    if (basedim32 != dim)
    {
        pageann::cout << "Error. dimensions dont match for train set and base set" << std::endl;
        return -1;
    }
    ...
    for (size_t block = 0; block < num_blocks; block++)
    {
        ...
        pageann::convert_types<T, float>(block_data_T.get(), block_data_float.get(), cur_blk_size, dim);
    ...
    pageann::cout << "Actual shard sizes: " << std::flush;
    for (size_t i = 0; i < num_centers; i++)
    {
        uint32_t cur_shard_count = (uint32_t)shard_counts[i];
...
int retrieve_shard_data_from_ids(const std::string data_file, std::string idmap_filename, std::string data_filename)
{
    ...
    pageann::load_bin<uint32_t>(idmap_filename, shard_ids, shard_size, tmp);
    ...
    pageann::cout << "Written file with " << num_written << " points" << std::endl;
              ...
              const std::string prefix_path, size_t k_base)
{
    ...
    pageann::cout << "Processing global k-means (kmeans_partitioning Step)" << std::endl;
    ...
    pageann::cout << "Saving global k-center pivots" << std::endl;
    pageann::save_bin<float>(output_file.c_str(), pivot_data, (size_t)num_parts, train_dim);
    ...
    while (!fit_in_ram)
    {
        ...
        for (auto &p : cluster_sizes)
        {
            ...
            double cur_shard_ram_estimate =
                pageann::estimate_ram_usage(p, (uint32_t)train_dim, sizeof(T), (uint32_t)graph_degree);

            if (cur_shard_ram_estimate > max_ram_usage)
                max_ram_usage = cur_shard_ram_estimate;
        }
        pageann::cout << "With " << num_parts
