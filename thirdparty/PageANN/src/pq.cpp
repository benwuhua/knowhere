...
void FixedChunkPQTable::load_pq_centroid_bin(const char *pq_table_file, size_t &num_chunks)
{
    ...
    size_t *file_offset_data; // since load_bin only sets the pointer, no need
                              // to delete.
    pageann::load_bin<size_t>(files, pq_table_file, file_offset_data, nr, nc);
#else
    ...
    pageann::load_bin<size_t>(pq_table_file, file_offset_data, nr, nc);//get the metadata first; the metadata is the file_offset_data; note: not chunk_offset
#endif
...
//offset[3] is the end of file -- basically wont be used at all
    if (nr != 4 && nr != 5)
    {
        pageann::cout << "Error reading pq_pivots file " << pq_table_file
                      << ". Offsets dont contain correct metadata, # offsets = " << nr << ", but expecting " << 4
                      << " or " << 5;
        throw pageann::ANNException("Error reading pq_pivots file at offsets data.", -1, __FUNCSIG__, __FILE__,
                                    __LINE__);
    }

    if (nr == 4)
    {
        pageann::cout << "Offsets: " << file_offset_data[0] << " " << file_offset_data[1] << " " << file_offset_data[2]
    ...
    {
        throw pageann::ANNException("Wrong number of offsets in pq_pivots", -1, __FUNCSIG__, __FILE__, __LINE__);
    }

#ifdef EXEC_ENV_OLS
    pageann::load_bin<float>(files, pq_table_file, tables, nr, nc, file_offset_data[0]);
#else
    pageann::load_bin<float>(pq_table_file, tables, nr, nc, file_offset_data[0]);
#endif

    if ((nr != NUM_PQ_CENTROIDS))
    {
        pageann::cout << "Error reading pq_pivots file " << pq_table_file << ". file_num_centers  = " << nr
                      << " but expecting " << NUM_PQ_CENTROIDS << " centers";
        throw pageann::ANNException("Error reading pq_pivots file at pivots data.", -1, __FUNCSIG__, __FILE__,
                                    __LINE__);
    }
...
#ifdef EXEC_ENV_OLS
    pageann::load_bin<float>(files, pq_table_file, centroid, nr, nc, file_offset_data[1]);
#else
    pageann::load_bin<float>(pq_table_file, centroid, nr, nc, file_offset_data[1]);
#endif

    if ((nr != this->ndims) || (nc != 1))
    {
        pageann::cerr << "Error reading centroids from pq_pivots file " << pq_table_file << ". file_dim  = " << nr
                      << ", file_cols = " << nc << " but expecting " << this->ndims << " entries in 1 dimension.";
        throw pageann::ANNException("Error reading pq_pivots file at centroid data.", -1, __FUNCSIG__, __FILE__,
                                    __LINE__);
    }
...
#ifdef EXEC_ENV_OLS
    pageann::load_bin<uint32_t>(files, pq_table_file, chunk_offsets, nr, nc, file_offset_data[chunk_offsets_index]);
#else
    pageann::load_bin<uint32_t>(pq_table_file, chunk_offsets, nr, nc, file_offset_data[chunk_offsets_index]);
#endif

    // if (nc != 1 || (nr != num_chunks + 1 && num_chunks != 0))
    // {
    //     pageann::cerr << "Error loading chunk offsets file. numc: " << nc << " (should be 1). numr: " << nr
    //                   << " (should be " << num_chunks + 1 << " or 0 if we need to infer)" << std::endl;
    //     throw pageann::ANNException("Error loading chunk offsets file", -1, __FUNCSIG__, __FILE__, __LINE__);
    // }
    ...
    pageann::cout << "Loaded PQ Pivots: #ctrs: " << NUM_PQ_CENTROIDS << ", #dims: " << this->ndims
                  << ", #chunks: " << this->n_chunks << std::endl;

#ifdef EXEC_ENV_OLS
    if (files.fileExists(rotmat_file))
    {
        pageann::load_bin<float>(files, rotmat_file, (float *&)rotmat_tr, nr, nc);
#else
    if (file_exists(rotmat_file))
    {
        pageann::load_bin<float>(rotmat_file, rotmat_tr, nr, nc);
#endif
        if (nr != this->ndims || nc != this->ndims)
        {
            pageann::cerr << "Error loading rotation matrix file" << std::endl;
            throw pageann::ANNException("Error loading rotation matrix file", -1, __FUNCSIG__, __FILE__, __LINE__);
        }
        use_rotation = true;
    }
...
//pageann::aggregate_coords(ids, this->data, this->_n_chunks, pq_coord_scratch); 
...
//pageann::pq_dist_lookup(pq_coord_scratch, n_ids, this->_n_chunks, pq_dists, dists_out);
...
// functions above
//pageann::aggregate_coords(ids, n_ids, this->data, this->_n_chunks, pq_coord_scratch);
                       ...
                       bool make_zero_mean)
{
    ...
    {
        pageann::cout << " Error: number of chunks more than dimension" << std::endl;
        return -1;
    }
    ...
    if (file_exists(pq_pivots_path))
    {
        ...
        pageann::load_bin<float>(pq_pivots_path, full_pivot_data, file_num_centers, file_dim, METADATA_SIZE);
        if (file_dim == dim && file_num_centers == num_centers)
        {
            pageann::cout << "PQ pivot file exists. Not generating again" << std::endl;
            return -1;
        }
    }
...
//bin can be understood as a segment
    ...
    for (size_t i = 0; i < num_pq_chunks; i++)
    {
        ...
        pageann::cout << "Processing chunk " << i << " with dimensions [" << chunk_offsets[i] << ", "
...
///MARK: update the cluster centers. and stop If the relative change in residuals is less than 0.00001 or the residual is less than machine epsilon, print a message and terminate early
    ...
    cumul_bytes[1] = cumul_bytes[0] + pageann::save_bin<float>(pq_pivots_path.c_str(), full_pivot_data.get(),
                                                               (size_t)num_centers, dim, cumul_bytes[0]);
    ///offset[1]: this is the offset for centroid     
    cumul_bytes[2] = cumul_bytes[1] +
                     pageann::save_bin<float>(pq_pivots_path.c_str(), centroid.get(), (size_t)dim, 1, cumul_bytes[1]);

    ///offset[2]: this is the offset for reading chunk_offsets
    cumul_bytes[3] = cumul_bytes[2] + pageann::save_bin<uint32_t>(pq_pivots_path.c_str(), chunk_offsets.data(),
                                                                  chunk_offsets.size(), 1, cumul_bytes[2]);
    //offset at 0: save this dataoffset in the begninning
    pageann::save_bin<size_t>(pq_pivots_path.c_str(), cumul_bytes.data(), cumul_bytes.size(), 1, 0);

    pageann::cout << "Saved pq pivot data to " << pq_pivots_path << " of size " << cumul_bytes[cumul_bytes.size() - 1]
                  << "B." << std::endl;
...
#pragma omp parallel for schedule(static, 65536)
    ...
    cumul_bytes[1] = cumul_bytes[0] + pageann::save_bin<float>(opq_pivots_path.c_str(), full_pivot_data.get(),
                                                               (size_t)num_centers, dim, cumul_bytes[0]);
    cumul_bytes[2] = cumul_bytes[1] +
                     pageann::save_bin<float>(opq_pivots_path.c_str(), centroid.get(), (size_t)dim, 1, cumul_bytes[1]);
    cumul_bytes[3] = cumul_bytes[2] + pageann::save_bin<uint32_t>(opq_pivots_path.c_str(), chunk_offsets.data(),
                                                                  chunk_offsets.size(), 1, cumul_bytes[2]);
    pageann::save_bin<size_t>(opq_pivots_path.c_str(), cumul_bytes.data(), cumul_bytes.size(), 1, 0);

    pageann::cout << "Saved opq pivot data to " << opq_pivots_path << " of size " << cumul_bytes[cumul_bytes.size() - 1]
                  << "B." << std::endl;

    std::string rotmat_path = opq_pivots_path + "_rotation_matrix.bin";
    pageann::save_bin<float>(rotmat_path.c_str(), rotmat_tr.get(), dim, dim);
                                 ...
                                 bool use_opq)
{
    ...
    if (!file_exists(pq_pivots_path))
    {
        ...
        throw pageann::ANNException("PQ k-means pivot file not found", -1);
    }
    else
    {
        size_t nr, nc;
        ...
        pageann::load_bin<size_t>(pq_pivots_path.c_str(), file_offset_data, nr, nc, 0);

        if (nr != 4)
        {
            pageann::cout << "Error reading pq_pivots file " << pq_pivots_path
                          << ". Offsets dont contain correct metadata, # offsets = " << nr << ", but expecting 4.";
            throw pageann::ANNException("Error reading pq_pivots file at offsets data.", -1, __FUNCSIG__, __FILE__,
                                        __LINE__);
        }

        pageann::load_bin<float>(pq_pivots_path.c_str(), full_pivot_data, nr, nc, file_offset_data[0]);

        if ((nr != num_centers) || (nc != dim))
        {
            pageann::cout << "Error reading pq_pivots file " << pq_pivots_path << ". file_num_centers  = " << nr
        ...
        pageann::load_bin<float>(pq_pivots_path.c_str(), centroid, nr, nc, file_offset_data[1]);

        if ((nr != dim) || (nc != 1))
        {
            pageann::cout << "Error reading pq_pivots file " << pq_pivots_path << ". file_dim  = " << nr
                          << ", file_cols = " << nc << " but expecting " << dim << " entries in 1 dimension.";
            throw pageann::ANNException("Error reading pq_pivots file at centroid data.", -1, __FUNCSIG__, __FILE__,
                                        __LINE__);
        }

        pageann::load_bin<uint32_t>(pq_pivots_path.c_str(), chunk_offsets, nr, nc, file_offset_data[2]);

        if (nr != (uint64_t)num_pq_chunks + 1 || nc != 1)
        {
            pageann::cout << "Error reading pq_pivots file at chunk offsets; file has nr=" << nr << ",nc=" << nc
                          << ", expecting nr=" << num_pq_chunks + 1 << ", nc=1." << std::endl;
            throw pageann::ANNException("Error reading pq_pivots file at chunk offsets.", -1, __FUNCSIG__, __FILE__,
                                        __LINE__);
        }
        ...
        if (use_opq)
        {
            ...
            pageann::load_bin<float>(rotmat_path.c_str(), rotmat_tr, nr, nc);
            if (nr != (uint64_t)dim || nc != dim)
            {
                pageann::cout << "Error reading rotation matrix file." << std::endl;
                throw pageann::ANNException("Error reading rotation matrix file.", -1, __FUNCSIG__, __FILE__, __LINE__);
            }
        }

        pageann::cout << "Loaded PQ pivot information" << std::endl;
    }
...
////MARK: BLOCK_SIZE is 5M --- we process in blocks because all data may not be able to loaded into RAM
    for (size_t block = 0; block < num_blocks; block++)
    {
        ...
        pageann::convert_types<T, float>(block_data_T.get(), block_data_tmp.get(), cur_blk_size, dim);

        pageann::cout << "Processing points  [" << start_id << ", " << end_id << ").." << std::flush;
...
#pragma omp parallel for schedule(static, 8192)
        ...
        else
        {
            ...
            pageann::convert_types<uint32_t, uint8_t>(block_compressed_base.get(), pVec.get(), cur_blk_size,
                                                      num_pq_chunks);
...
void generate_disk_quantized_data(const std::string &data_file_to_use, const std::string &disk_pq_pivots_path,
                                  const std::string &disk_pq_compressed_vectors_path, pageann::Metric compareMetric,
                                  const double p_val, size_t &disk_pq_dims)
{
    ...
    pageann::cout << "Training data with " << train_size << " samples loaded." << std::endl;
...
void generate_quantized_data(const std::string &data_file_to_use, const std::string &pq_pivots_path,
                             const std::string &pq_compressed_vectors_path, pageann::Metric compareMetric,
                             ...
                             const std::string &codebook_prefix)
{
    ...
    {
        pageann::cout << "Skip Training with predefined pivots in: " << pq_pivots_path << std::endl;
    }
...
template DISKANN_DLLEXPORT void generate_disk_quantized_data<int8_t>(const std::string &data_file_to_use,
                                                                     ...
                                                                     pageann::Metric compareMetric, const double p_val,
                                                                     size_t &disk_pq_dims);
...
template DISKANN_DLLEXPORT void generate_disk_quantized_data<uint8_t>(
    ...
    const std::string &disk_pq_compressed_vectors_path, pageann::Metric compareMetric, const double p_val,
    size_t &disk_pq_dims);
