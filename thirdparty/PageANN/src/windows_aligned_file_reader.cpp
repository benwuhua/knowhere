...
void WindowsAlignedFileReader::register_thread()
{
    ...
    {
        pageann::cout << "Warning:: Duplicate registration for thread_id : " << std::this_thread::get_id() << std::endl;
    }
    ...
    if (ctx.fhandle == INVALID_HANDLE_VALUE)
    {
        ...
        {
            pageann::cout << "Error opening " << filePath << " -- error=" << GetLastError() << std::endl;
        }
        else
        {
            pageann::cout << "Error converting wchar to char -- error=" << GetLastError() << std::endl;
        }
    }
...
IOContext &WindowsAlignedFileReader::get_ctx()
{
    ...
    if (ctx_map.find(std::this_thread::get_id()) == ctx_map.end())
    {
        ...
        throw pageann::ANNException(stream.str(), -2, __FUNCSIG__, __FILE__, __LINE__);
    }
...
void WindowsAlignedFileReader::read(std::vector<AlignedRead> &read_reqs, IOContext &ctx, bool async)
{
    ...
    //     pageann::cerr << "Failed to get file size, error code: " << error << std::endl;
    //     throw pageann::ANNException("Failed to get file size", error, __FUNCSIG__, __FILE__, __LINE__);
    // }
    ...
    for (uint64_t i = 0; i < n_batches; i++)
    {
        ...
        for (auto &os : ctx.reqs)
        {
            ...
            /*
              if (ResetEvent(os.hEvent) == 0) {
                pageann::cerr << "ResetEvent failed" << std::endl;
                exit(-3);
              }
            */
        }
        //pageann::cout << "p2.1 " << std::endl;
        ...
        for (uint64_t j = 0; j < batch_size; j++)
        {
            ...
            //     pageann::cerr << "Attempt to read beyond file size. Offset: " << offset << ", Size: " << nbytes << ", File size: " << file_size.QuadPart << std::endl;
            //     throw pageann::ANNException("Attempt to read beyond file size", ERROR_HANDLE_EOF, __FUNCSIG__, __FILE__, __LINE__);
            // }
            ...
            if (ctx.fhandle == INVALID_HANDLE_VALUE) {
                pageann::cerr << "Invalid file handle" << std::endl;
                throw pageann::ANNException("Invalid file handle", ERROR_INVALID_HANDLE, __FUNCSIG__, __FILE__, __LINE__);
            }
            ...
            if (ret == FALSE)
            {
                ...
                if (error != ERROR_IO_PENDING)
                {
                    ...
                    pageann::cerr << "Error queuing IO -- " << error << "\n";
                }
            }
            else
            {
                pageann::cerr << "Error queueing IO -- ReadFile returned TRUE" << std::endl;
            }
        }
        //pageann::cout << "p2.2 " << std::endl;
        DWORD n_read = 0;
        ...
        while (n_complete < batch_size)
        {
            if (GetQueuedCompletionStatus(ctx.iocp, &n_read, &completion_key, &lp_os, INFINITE) != 0)
            {
                ...
                n_complete++;
                //pageann::cout << "p2.3-success " << std::endl;
            }
            else
            {
                ...
                if (lp_os == NULL)
                {
                    //pageann::cout << "p2.3.1-fail " << std::endl;
                    DWORD error = GetLastError();
                    if (error != WAIT_TIMEOUT)
                    {
                        pageann::cerr << "GetQueuedCompletionStatus() failed "
                                         "with error = "
                                      << error << std::endl;
                        throw pageann::ANNException("GetQueuedCompletionStatus failed with error: ", error, __FUNCSIG__,
                                                    __FILE__, __LINE__);
                    }
                ...
                else
                {
                    ...
                    //pageann::cout << "p2.3.2-fail " << std::endl;
        ...
        //pageann::cout << "p2.4 " << std::endl;
    }
    //pageann::cout << "end " << std::endl;
}
#endif
#endif
