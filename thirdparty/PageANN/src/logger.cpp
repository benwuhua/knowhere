...
{
    g_logger = logger;
    pageann::cout << "Set Custom Logger" << std::endl;
}
...
ANNStreamBuf::ANNStreamBuf(FILE *fp)
{
    if (fp == nullptr)
    {
        throw pageann::ANNException("File pointer passed to ANNStreamBuf() cannot be null", -1);
    }
    if (fp != stdout && fp != stderr)
    {
        throw pageann::ANNException("The custom logger only supports stdout and stderr.", -1);
    }
    _fp = fp;
...
{
    throw pageann::ANNException("Attempt to read on streambuf meant only for writing.", -1);
}
