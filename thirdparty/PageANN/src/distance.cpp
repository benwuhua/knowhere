...
template <typename T> pageann::Metric Distance<T>::get_metric() const
{
...
{
#ifdef _WINDOWS
    return pageann::CosineSimilarity2<int8_t>(a, b, length);
#else
...
{
#ifdef _WINDOWS
    return pageann::CosineSimilarity2<float>(a, b, length);
#else
...
template <typename T> float DistanceInnerProduct<T>::inner_product(const T *a, const T *b, uint32_t size) const
{
    ...
    {
        pageann::cerr << "ERROR: Inner Product only defined for float currently." << std::endl;
        throw pageann::ANNException("ERROR: Inner Product only defined for float currently.", -1, __FUNCSIG__, __FILE__,
                                    __LINE__);
    }
...
template <typename T> float DistanceFastL2<T>::norm(const T *a, uint32_t size) const
{
    if (!std::is_floating_point<T>::value)
    {
        pageann::cerr << "ERROR: FastL2 only defined for float currently." << std::endl;
        throw pageann::ANNException("ERROR: FastL2 only defined for float currently.", -1, __FUNCSIG__, __FILE__,
                                    __LINE__);
    }
    float result = 0;
#ifdef __GNUC__
...
template <> pageann::Distance<float> *get_distance_function(pageann::Metric m)
{
    if (m == pageann::Metric::L2)
    {
        if (Avx2SupportedCPU)
        {
            pageann::cout << "L2: Using AVX2 distance computation DistanceL2Float" << std::endl;
            return new pageann::DistanceL2Float();
        }
        else if (AvxSupportedCPU)
        {
            pageann::cout << "L2: AVX2 not supported. Using AVX distance computation" << std::endl;
            return new pageann::AVXDistanceL2Float();
        }
        else
        {
            pageann::cout << "L2: Older CPU. Using slow distance computation" << std::endl;
            return new pageann::SlowDistanceL2<float>();
        }
    }
    else if (m == pageann::Metric::COSINE)
    {
        pageann::cout << "Cosine: Using either AVX or AVX2 implementation" << std::endl;
        return new pageann::DistanceCosineFloat();
    }
    else if (m == pageann::Metric::INNER_PRODUCT)
    {
        pageann::cout << "Inner product: Using AVX2 implementation "
                         "AVXDistanceInnerProductFloat"
                      << std::endl;
        return new pageann::AVXDistanceInnerProductFloat();
    }
    else if (m == pageann::Metric::FAST_L2)
    {
        pageann::cout << "Fast_L2: Using AVX2 implementation with norm "
                         "memoization DistanceFastL2<float>"
                      << std::endl;
        return new pageann::DistanceFastL2<float>();
    }
    else
    {
...
template <> pageann::Distance<int8_t> *get_distance_function(pageann::Metric m)
{
    if (m == pageann::Metric::L2)
    {
        if (Avx2SupportedCPU)
        {
            pageann::cout << "Using AVX2 distance computation DistanceL2Int8." << std::endl;
            return new pageann::DistanceL2Int8();
        }
        else if (AvxSupportedCPU)
        {
            pageann::cout << "AVX2 not supported. Using AVX distance computation" << std::endl;
            return new pageann::AVXDistanceL2Int8();
        }
        else
        {
            pageann::cout << "Older CPU. Using slow distance computation "
                             "SlowDistanceL2Int<int8_t>."
                          << std::endl;
            return new pageann::SlowDistanceL2<int8_t>();
        }
    }
    else if (m == pageann::Metric::COSINE)
    {
        pageann::cout << "Using either AVX or AVX2 for Cosine similarity "
                         "DistanceCosineInt8."
                      << std::endl;
        return new pageann::DistanceCosineInt8();
    }
    else
    {
        std::stringstream stream;
...
template <> pageann::Distance<uint8_t> *get_distance_function(pageann::Metric m)
{
    if (m == pageann::Metric::L2)
    {
#ifdef _WINDOWS
        pageann::cout << "WARNING: AVX/AVX2 distance function not defined for Uint8. "
                         "Using "
                         "slow version. "
                         "Contact gopalsr@microsoft.com if you need AVX/AVX2 support."
                      << std::endl;
#endif
        return new pageann::DistanceL2UInt8();
    }
    else if (m == pageann::Metric::COSINE)
    {
        pageann::cout << "AVX/AVX2 distance function not defined for Uint8. Using "
                         "slow version SlowDistanceCosineUint8() "
                         "Contact gopalsr@microsoft.com if you need AVX/AVX2 support."
                      << std::endl;
        return new pageann::SlowDistanceCosineUInt8();
    }
    else
    {
        std::stringstream stream;
