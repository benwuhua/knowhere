                          ...
                          bool transpose_rot)
{
    ...
    if (transpose_rot)
    {
        pageann::cout << "Transposing rotation matrix.." << std::flush;
        transpose = CblasTrans;
    }
    pageann::cout << "done Rotating data with random matrix.." << std::flush;
                                      ...
                                      uint32_t *center_index, float *const dist_matrix, size_t k)
{
    if (k > num_centers)
    {
        pageann::cout << "ERROR: k (" << k << ") > num_center(" << num_centers << ")" << std::endl;
        return;
    }
...
{
    pageann::cout << "Processing residuals of " << num_points << " points in " << dim << " dimensions using "
                 ...
                 const size_t max_reps, std::vector<size_t> *closest_docs, uint32_t *closest_center)
{
    ...
    for (size_t i = 0; i < max_reps; ++i)
    {
        ...
        {
            pageann::cout << "Residuals unchanged: " << old_residual << " becomes " << residual
...
void kmeanspp_selecting_pivots(float *data, size_t num_points, size_t dim, float *pivot_data, size_t num_centers)
{
    if (num_points > 1 << 23)
    {
        pageann::cout << "ERROR: n_pts " << num_points
