...
void IndexFactory::check_config()
{
    ...
    if (_config->pq_dist_build)
    {
        ...
        if (_config->metric == pageann::Metric::INNER_PRODUCT)
...
{
    if (metric == pageann::Metric::COSINE && std::is_same<T, float>::value)
    {
                                                                        ...
                                                                        Metric metric)
{
    ...
    case DataStoreStrategy::MEMORY:
        ...
        return std::make_shared<pageann::InMemDataStore<T>>((location_t)total_internal_points, dimension,
                                                            std::move(distance));
    default:
        break;
    }
                                                                        ...
                                                                        Metric metric)
{
    ...
    case DataStoreStrategy::MEMORY:
        distance.reset(construct_inmem_distance_fn<T>(metric));
        return std::make_shared<pageann::InMemOOCDataStore<T>>((location_t)total_internal_points, dimension,
                                                            std::move(distance));
    default:
        break;
    }
                                                                     ...
                                                                     bool use_opq)
{
    ...
    case DataStoreStrategy::MEMORY:
        ...
        return std::make_shared<pageann::PQDataStore<T>>(dimension, (location_t)(num_points), num_pq_chunks,
...
std::unique_ptr<AbstractIndex> IndexFactory::create_instance()
{
    ...
    return std::make_unique<pageann::Index<data_type, tag_type, label_type>>(*_config, data_store,
