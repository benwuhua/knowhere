      ...
      _distance_metric(distance_fn->get_metric())
{
    ...
    {
        throw pageann::ANNException("ERROR: num_pq_chunks > dim", -1, __FUNCSIG__, __FILE__, __LINE__);
    }
...
{
    return pageann::save_bin(filename, _quantized_data, this->capacity(), _num_chunks, 0);
}
...
template <typename data_t> void PQDataStore<data_t>::prefetch_vector(const location_t loc)
{
    ...
    pageann::prefetch_vector((const char *)ptr, _num_chunks * sizeof(data_t));
}
...
void PQDataStore<data_t>::preprocess_query(const data_t *aligned_query, AbstractScratch<data_t> *scratch) const
{
    if (scratch == nullptr)
    {
        throw pageann::ANNException("Scratch space is null", -1);
    }
    ...
    {
        throw pageann::ANNException("PQScratch space has not been set in the scratch object.", -1);
    }
                                       ...
                                       AbstractScratch<data_t> *scratch_space) const
{
    ...
    if (pq_scratch == nullptr)
    {
        throw pageann::ANNException("PQScratch not set in scratch space.", -1);
    }
    pageann::aggregate_coords(locations, location_count, _quantized_data, this->_num_chunks,
                                       ...
                                       std::vector<float> &distances, AbstractScratch<data_t> *scratch_space) const
{
    ...
    pageann::aggregate_coords(ids, _quantized_data, this->_num_chunks, pq_scratch->aligned_pq_coord_scratch);
