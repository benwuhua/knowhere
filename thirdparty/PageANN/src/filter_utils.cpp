...
{
    pageann::IndexWriteParameters label_index_build_parameters = pageann::IndexWriteParametersBuilder(L, R)
    ...
    pageann::cout.setstate(std::ios_base::failbit);
    for (const auto &lbl : all_labels)
    {
        ...
        pageann::get_bin_metadata(curr_label_input_data_path, number_of_label_points, dimension);

        pageann::Index<T> index(pageann::Metric::L2, dimension, number_of_label_points,
                                std::make_shared<pageann::IndexWriteParameters>(label_index_build_parameters), nullptr,
...
std::tuple<std::vector<std::vector<LabelT>>, tsl::robin_set<LabelT>> parse_formatted_label_file(std::string label_file)
{
    ...
    pageann::cout << "Identified " << labels.size() << " distinct label(s)" << std::endl;
