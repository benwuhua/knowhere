...
std::tuple<uint32_t, uint32_t, size_t> InMemGraphStore::load_impl(AlignedFileReader &reader, size_t expected_num_points)
{
    ...
    pageann::cout << "done. Index has " << nodes_read << " nodes and " << cc << " out-edges, _start is set to " << start
                  << std::endl;
                                                                  ...
                                                                  size_t expected_num_points)
{
    ...
    pageann::cout << "Loading vamana graph " << filename << "..." << std::flush;
    ...
    while (bytes_read != expected_file_size)
    {
        ...
        if (k == 0)
        {
            pageann::cerr << "ERROR: Point found with no out-neighbours, point#" << nodes_read << std::endl;
        }
