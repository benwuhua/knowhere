    ...
    : _L(0), _R(r), _maxc(maxc)
{
    if (search_l == 0 || indexing_l == 0 || r == 0 || dim == 0)
    {
        ...
        throw pageann::ANNException(ss.str(), -1);
    }
...
template <typename T> SSDQueryScratch<T>::SSDQueryScratch(size_t aligned_dim, size_t visited_reserve, size_t nnodes_sector)
{
    ...
    pageann::alloc_aligned((void **)&coord_scratch, coord_alloc_size, 256);
    pageann::alloc_aligned((void **)&sector_scratch, defaults::MAX_N_SECTOR_READS * defaults::SECTOR_LEN,
                           defaults::SECTOR_LEN);////const uint64_t MAX_N_SECTOR_READS = 128;
    ///MARK: it needs to aligned with 0 cuz AXV load 8 floats at a time
    pageann::alloc_aligned((void **)&this->_aligned_query_T, aligned_dim * sizeof(T), 8 * sizeof(T)); 
...
{
    pageann::aligned_free((void *)coord_scratch);
    pageann::aligned_free((void *)sector_scratch);
    pageann::aligned_free((void *)this->_aligned_query_T);
...
{   
    ...
    pageann::alloc_aligned((void **)&aligned_pq_coord_scratch, (size_t)graph_degree * (size_t)MAX_PQ_CHUNKS * sizeof(uint8_t), 256);
    ...
    pageann::alloc_aligned((void **)&aligned_pqtable_dist_scratch, 256 * (size_t)MAX_PQ_CHUNKS * sizeof(float), 256);
    //pq distances of neibors to the current node
    pageann::alloc_aligned((void **)&aligned_dist_scratch, (size_t)graph_degree * sizeof(float), 256);
    //query's real value
    pageann::alloc_aligned((void **)&aligned_query_float, aligned_dim * sizeof(float), 8 * sizeof(float));
    pageann::alloc_aligned((void **)&rotated_query, aligned_dim * sizeof(float), 8 * sizeof(float));
...
{
    pageann::aligned_free((void *)aligned_pq_coord_scratch);
    pageann::aligned_free((void *)aligned_pqtable_dist_scratch);
    pageann::aligned_free((void *)aligned_dist_scratch);
    pageann::aligned_free((void *)aligned_query_float);
    pageann::aligned_free((void *)rotated_query);
}
