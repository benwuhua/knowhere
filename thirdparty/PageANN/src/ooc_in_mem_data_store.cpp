...
template <typename data_t> location_t InMemOOCDataStore<data_t>::load_impl(const std::string &filename)
{
    ...
    pageann::cout << "data_file_num_points: " << file_num_points << std::endl;
    pageann::cout << "data_file_dim: " << file_dim << std::endl;
    pageann::cout << "npts_to_cache: " << this->capacity() << std::endl << std::endl;
    copy_aligned_data_from_file<data_t>(filename.c_str(), _data, file_num_points, file_dim, _aligned_dim);
    pageann::cout << "Finish loading data points to _data " << std::endl;
...
template <typename data_t> void InMemOOCDataStore<data_t>::populate_data(const std::string &filename, const size_t offset)
{
    ...
    if (_distance_fn->preprocessing_required())
    {
         pageann::cout << "Preprocess (normalize) data when populating data from file" << std::endl;
...
{
    pageann::prefetch_vector((const char *)_data + _aligned_dim * (size_t)loc * sizeof(data_t),
