      ...
      _delete_set(new tsl::robin_set<uint32_t>), _conc_consolidate(index_config.concurrent_consolidate)
{
    ...
    if (_pq_dist)
    {
        ...
        if (_dist_metric == pageann::Metric::INNER_PRODUCT)
...
template <typename T, typename TagT, typename LabelT> size_t Index<T, TagT, LabelT>::save_tags(std::string tags_file)
{
    if (!_enable_tags)
    {
        pageann::cout << "Not saving tags as they are not enabled." << std::endl;
        return 0;
    }
...
void Index<T, TagT, LabelT>::save(const char *filename, bool compact_before_save)
{
    ...
    if (!_save_as_one_file)
    {
        if (_filtered_index)
        {
            if (_label_to_start_id.size() > 0)
            {
                ...
                {
                    throw pageann::ANNException(std::string("Failed to open file ") + filename, -1);
                }
    ...
    {
        pageann::cout << "Save index in a single file currently not supported. "
    ...
    pageann::cout << "Time taken for save: " << timer.elapsed() / 1000000.0 << "s." << std::endl;
}
...
size_t Index<T, TagT, LabelT>::load_tags(const std::string tag_filename)
{
    ...
    {
        pageann::cerr << "Tag file " << tag_filename << " does not exist!" << std::endl;
        throw pageann::ANNException("Tag file " + tag_filename + " does not exist!", -1, __FUNCSIG__, __FILE__,
                                    __LINE__);
    }
#endif
    if (!_enable_tags)
    {
        pageann::cout << "Tags not loaded as tags not enabled." << std::endl;
        return 0;
    }
    ...
    pageann::cout << "Tags loaded." << std::endl;
    delete[] tag_data;
...
size_t Index<T, TagT, LabelT>::load_data(std::string filename)
{
    ...
    size_t file_dim, file_num_points;
#ifdef EXEC_ENV_OLS
    pageann::get_bin_metadata(reader, file_num_points, file_dim);
#else
    ...
    pageann::get_bin_metadata(filename, file_num_points, file_dim);
#endif
...
#ifdef EXEC_ENV_OLS
    pageann::load_bin<uint32_t>(reader, delete_list, npts, ndim);
#else
    pageann::load_bin<uint32_t>(filename, delete_list, npts, ndim);
#endif
    assert(ndim == 1);
...
void Index<T, TagT, LabelT>::load(const char *filename, uint32_t num_threads, uint32_t search_l)
{
    ...
    {
        pageann::cout << "Single index file saving/loading support not yet "
    ...
    reposition_frozen_point_to_end();
    pageann::cout << "Num frozen points:" << _num_frozen_pts << " _nd: " << _nd << " _start: " << _start
...
template <typename T, typename TagT, typename LabelT> int Index<T, TagT, LabelT>::get_vector_by_tag(TagT &tag, T *vec)
{
    ...
    {
        pageann::cout << "Tag " << get_tag_string(tag) << " does not exist" << std::endl;
        return -1;
    }
    ...
    const std::vector<LabelT> &filter_labels, bool search_invocation)
{
    ...
    for (auto id : init_ids)
    {
        ...
        {
            pageann::cerr << "Out of range loc found as an edge : " << id << std::endl;
            throw pageann::ANNException(std::string("Wrong loc") + std::to_string(id), -1, __FUNCSIG__, __FILE__,
                                        __LINE__);
        }
                                                        ...
                                                        uint32_t filteredLindex)
{
    ...
    {
        throw pageann::ANNException("ERROR: non-empty pruned_list passed", -1, __FUNCSIG__, __FILE__, __LINE__);
    }
    ...
    while (cur_alpha <= alpha && result.size() < degree)
    {
        ...
        for (auto iter = pool.begin(); result.size() < degree && iter != pool.end(); ++iter)
        {
            ...
            for (auto iter2 = iter + 1; iter2 != pool.end(); iter2++)
            {
                ...
                if (_dist_metric == pageann::Metric::L2 || _dist_metric == pageann::Metric::COSINE)
                {
                    occlude_factor[t] = (djk == 0) ? std::numeric_limits<float>::max()
                                                   : std::max(occlude_factor[t], iter2->distance / djk);
                }
                else if (_dist_metric == pageann::Metric::INNER_PRODUCT)
                {
...
template <typename T, typename TagT, typename LabelT> void Index<T, TagT, LabelT>::link()
{
    ...
    std::vector<pageann::Neighbor> pool, tmp;
...
#pragma omp parallel for schedule(dynamic, 2048)
    for (int64_t node_ctr = 0; node_ctr < (int64_t)(visit_order.size()); node_ctr++)
    {
        ...
        {
            pageann::cout << "\r" << (100.0 * node_ctr) / (visit_order.size()) << "% of index build completed."
                          << std::flush;
        }
    }
    ...
    if (_nd > 0)
    {
        pageann::cout << "Starting final cleanup.." << std::flush;
    }
#pragma omp parallel for schedule(dynamic, 2048)
    for (int64_t node_ctr = 0; node_ctr < (int64_t)(visit_order.size()); node_ctr++)
    {
    ...
    if (_nd > 0)
    {
        pageann::cout << "done. Link time: " << ((double)link_timer.elapsed() / (double)1000000) << "s" << std::endl;
    }
}
...
#pragma omp parallel for
    ...
    pageann::cout << "Prune time : " << timer.elapsed() / 1000 << "ms" << std::endl;
    ...
    if (_nd > 0)
    {
        pageann::cout << "Index built with degree: max:" << max
...
void Index<T, TagT, LabelT>::set_start_points(const T *data, size_t data_count)
{
    ...
    _has_built = true;
    pageann::cout << "Index start points set: #" << _num_frozen_pts << std::endl;
}
...
{
    pageann::cout << "Starting index build with " << _nd << " points... " << std::endl;
    ...
    pageann::cout << "Index built with degree: max:" << max << "  avg:" << (float)total / (float)(_nd + _num_frozen_pts)
...
void Index<T, TagT, LabelT>::build(const char *filename, const size_t num_points_to_load, const std::vector<TagT> &tags)
{
    ...
    if (filename == nullptr)
    {
        throw pageann::ANNException("Can not build with an empty file", -1, __FUNCSIG__, __FILE__, __LINE__);
    }
    ...
    pageann::cout << "Begin to populate data into memory from file.. " << std::endl;
...
void Index<T, TagT, LabelT>::build(const char *filename, const size_t num_points_to_load, const char *tag_filename)
{
    ...
    if (_enable_tags)
    {
        ...
        else
        {
            ...
            {
                pageann::cout << "Loading tags from " << tag_filename << " for vamana index build" << std::endl;
                TagT *tag_data = nullptr;
                size_t npts, ndim;
                pageann::load_bin(tag_filename, tag_data, npts, ndim);
                if (npts < num_points_to_load)
                {
                    std::stringstream sstream;
                    sstream << "Loaded " << npts << " tags, insufficient to populate tags for " << num_points_to_load
                            << "  points to load";
                    throw pageann::ANNException(sstream.str(), -1, __FUNCSIG__, __FILE__, __LINE__);
                }
            ...
            {
                throw pageann::ANNException(std::string("Tag file") + tag_filename + " does not exist", -1, __FUNCSIG__,
                                            __FILE__, __LINE__);
            }
        }
    }
...
void Index<T, TagT, LabelT>::parse_label_file(const std::string &label_file, size_t &num_points)
{
    ...
    if (infile.fail())
    {
        throw pageann::ANNException(std::string("Failed to open file ") + label_file, -1);
    }
    ...
    pageann::cout << "Identified " << _labels.size() << " distinct label(s)" << std::endl;
}
                                                             ...
                                                             IdType *indices, float *distances)
{
    ...
    if (L > scratch->get_L())
    {
        pageann::cout << "Attempting to expand query scratch_space. Was created "
                      << "with Lsize: " << scratch->get_L() << " but search L is: " << L << std::endl;
        scratch->resize_for_new_L(L);
        pageann::cout << "Resize completed. New scratch->L is " << scratch->get_L() << std::endl;
    }
...
#else
                distances[pos] = _dist_metric == pageann::Metric::INNER_PRODUCT ? -1 * best_L_nodes[i].distance
    ...
    if (pos < K)
    {
        pageann::cerr << "Found pos: " << pos << "fewer than K elements " << K << " for query" << std::endl;
    }
                                                                          ...
                                                                          IdType *indices, float *distances)
{
    ...
    {
        pageann::cout << "No filtered medoid found. exitting "
                      << std::endl; // RKNOTE: If universal label found start there
        throw pageann::ANNException("No filtered medoid found. exitting ", -1);
    }
    if (_dynamic_index)
        tl.unlock();
    ...
    if (pos < K)
    {
        pageann::cerr << "Found fewer than K elements for query" << std::endl;
    }
...
{
    if (_num_frozen_pts == 0){
        pageann::cout << "There is no frozen point." << std::endl;
        return;
    }
...
template <typename T, typename TagT, typename LabelT> int Index<T, TagT, LabelT>::enable_delete()
{
    ...
    if (!_enable_tags)
    {
        pageann::cerr << "Tags must be instantiated for deletions" << std::endl;
        return -2;
    }
...
{
    if (!_enable_tags)
        throw pageann::ANNException("Point tag array not instantiated", -1, __FUNCSIG__, __FILE__, __LINE__);

    {
        ...
        if (_empty_slots.size() + _nd != _max_points)
        {
            ...
            pageann::cerr << err << std::endl;
            throw ANNException(err, -1, __FUNCSIG__, __FILE__, __LINE__);
        }

        if (_location_to_tag.size() + _delete_set->size() != _nd)
        {
            pageann::cerr << "Error: _location_to_tag.size (" << _location_to_tag.size() << ")  + _delete_set->size ("
                          << _delete_set->size() << ") != _nd(" << _nd << ") ";
            return consolidation_report(pageann::consolidation_report::status_code::INCONSISTENT_COUNT_ERROR, 0, 0, 0,
                                        0, 0, 0, 0);
        }

        if (_location_to_tag.size() != _tag_to_location.size())
        {
            throw pageann::ANNException("_location_to_tag and _tag_to_location not of same size", -1, __FUNCSIG__,
                                        __FILE__, __LINE__);
        }
    }
    ...
    if (!cl.try_lock())
    {
        pageann::cerr << "Consildate delete function failed to acquire consolidate lock" << std::endl;
        return consolidation_report(pageann::consolidation_report::status_code::LOCK_FAIL, 0, 0, 0, 0, 0, 0, 0);
    }

    pageann::cout << "Starting consolidate_deletes... ";
    ...
    {
        throw pageann::ANNException("ERROR: start node has been deleted", -1, __FUNCSIG__, __FILE__, __LINE__);
    }
...
#pragma omp parallel for num_threads(num_threads) schedule(dynamic, 8192) reduction(+ : num_calls_to_process_delete)
    ...
    pageann::cout << " done in " << duration << " seconds." << std::endl;
    return consolidation_report(pageann::consolidation_report::status_code::SUCCESS, ret_nd, max_points,
...
template <typename T, typename TagT, typename LabelT> void Index<T, TagT, LabelT>::compact_data()
{
    ...
    if (_data_compacted)
    {
        pageann::cerr << "Warning! Calling compact_data() when _data_compacted is true!" << std::endl;
        return;
    }
    ...
    {
        throw pageann::ANNException("ERROR: Start node deleted.", -1, __FUNCSIG__, __FILE__, __LINE__);
    }
    ...
    for (uint32_t old = 0; old < _max_points + _num_frozen_pts; ++old)
    {
            ...
            || (old >= _max_points && old < _max_points + _num_frozen_pts))
        {
            ...
            for (auto ngh_iter : _graph_store->get_neighbours((location_t)old))
            {
                ...
                {
                    ++num_dangling;
                    pageann::cerr << "Error in compact_data(). _final_graph[" << old << "] has neighbor " << ngh_iter
    ...
    pageann::cerr << "#dangling references after data compaction: " << num_dangling << std::endl;
    ...
    pageann::cout << "Time taken for compact_data: " << timer.elapsed() / 1000000. << "s." << std::endl;
}
...
template <typename T, typename TagT, typename LabelT> void Index<T, TagT, LabelT>::reposition_frozen_point_to_end()
{
    ...
    if (_nd == _max_points)
    {
        pageann::cout << "Not repositioning frozen point as it is already at the end." << std::endl;
        return;
    }
...
template <typename T, typename TagT, typename LabelT> void Index<T, TagT, LabelT>::resize(size_t new_max_points)
{
    ...
    pageann::cout << "Resizing took: " << std::chrono::duration<double>(stop - start).count() << "s" << std::endl;
}
...
int Index<T, TagT, LabelT>::insert_point(const T *point, const TagT tag, const std::vector<LabelT> &labels)
{
    ...
    if (tag == 0)
    {
        throw pageann::ANNException("Do not insert point with tag 0. That is "
    ...
    if (location == -1)
    {
        ...
        if (location == -1)
        {
            throw pageann::ANNException("Cannot reserve location even after "
...
template <typename T, typename TagT, typename LabelT> int Index<T, TagT, LabelT>::lazy_delete(const TagT &tag)
{
    ...
    if (_tag_to_location.find(tag) == _tag_to_location.end())
    {
        pageann::cerr << "Delete tag not found " << get_tag_string(tag) << std::endl;
        return -1;
    }
...
template <typename T, typename TagT, typename LabelT> void Index<T, TagT, LabelT>::print_status()
{
    ...
    pageann::cout << "------------------- Index object: " << (uint64_t)this << " -------------------" << std::endl;
    pageann::cout << "Number of points: " << _nd << std::endl;
    pageann::cout << "Graph size: " << _graph_store->get_total_points() << std::endl;
    pageann::cout << "Location to tag size: " << _location_to_tag.size() << std::endl;
    pageann::cout << "Tag to location size: " << _tag_to_location.size() << std::endl;
    pageann::cout << "Number of empty slots: " << _empty_slots.size() << std::endl;
    pageann::cout << std::boolalpha << "Data compacted: " << this->_data_compacted << std::endl;
    pageann::cout << "---------------------------------------------------------"
                     "------------"
                  << std::endl;
}
...
template <typename T, typename TagT, typename LabelT> void Index<T, TagT, LabelT>::count_nodes_at_bfs_levels()
{
    ...
    {
        pageann::cout << "Number of nodes at BFS level " << l << " is " << bfs_sets[l].size() << std::endl;
...
{ // use after build or load
    if (_dynamic_index)
    {
        throw pageann::ANNException("Optimize_index_layout not implemented for dyanmic indices", -1, __FUNCSIG__,
                                    __FILE__, __LINE__);
    }
