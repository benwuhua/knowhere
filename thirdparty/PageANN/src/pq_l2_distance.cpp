...
std::string PQL2Distance<data_t>::get_quantized_vectors_filename(const std::string &prefix) const
{
    if (_num_chunks == 0)
    {
        throw pageann::ANNException("Must set num_chunks before calling get_quantized_vectors_filename", -1,
                                    __FUNCSIG__, __FILE__, __LINE__);
    }
    return pageann::get_quantized_vectors_filename(prefix, _is_opq, (uint32_t)_num_chunks);
}
template <typename data_t> std::string PQL2Distance<data_t>::get_pivot_data_filename(const std::string &prefix) const
{
    if (_num_chunks == 0)
    {
        throw pageann::ANNException("Must set num_chunks before calling get_pivot_data_filename", -1, __FUNCSIG__,
                                    __FILE__, __LINE__);
    }
    return pageann::get_pivot_data_filename(prefix, _is_opq, (uint32_t)_num_chunks);
}
template <typename data_t>
std::string PQL2Distance<data_t>::get_rotation_matrix_suffix(const std::string &pq_pivots_filename) const
{
    return pageann::get_rotation_matrix_suffix(pq_pivots_filename);
}
...
void PQL2Distance<data_t>::load_pivot_data(const std::string &pq_table_file, size_t num_chunks)
{
    ...
    pageann::load_bin<size_t>(pq_table_file, file_offset_data, nr, nc);
#endif
...
#else
    pageann::load_bin<float>(pq_table_file, _tables, nr, nc, file_offset_data[0]);
#endif
...
#else
    pageann::load_bin<float>(pq_table_file, _centroid, nr, nc, file_offset_data[1]);
#endif
...
#else
    pageann::load_bin<uint32_t>(pq_table_file, _chunk_offsets, nr, nc, file_offset_data[chunk_offsets_index]);
#endif
    ...
    pageann::cout << "Loaded PQ Pivots: #ctrs: " << NUM_PQ_CENTROIDS << ", #dims: " << this->_ndims
...
#else
        pageann::load_bin<float>(rotmat_file, _rotmat_tr, nr, nc);
#endif
