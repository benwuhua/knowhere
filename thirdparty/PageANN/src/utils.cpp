...
void normalize_data_file(const std::string &inFileName, const std::string &outFileName)
{
    ...
    pageann::cout << "Normalizing FLOAT vectors in file: " << inFileName << std::endl;
    pageann::cout << "Dataset: #pts = " << npts << ", # dims = " << ndims << std::endl;

    size_t blk_size = 131072;
    size_t nblks = ROUND_UP(npts, blk_size) / blk_size;
    pageann::cout << "# blks: " << nblks << std::endl;
    ...
    pageann::cout << "Wrote normalized points to file: " << outFileName << std::endl;
}
                        ...
                        const tsl::robin_set<uint32_t> &active_tags)
{
    ...
    for (size_t i = 0; i < num_queries; i++)
    {
        ...
        {
            pageann::cout << "Warning: Couldn't find enough closest neighbors " << active_points_count << "/"
                          << recall_at
                          << " from "
                             "truthset for query # "
...
void get_bin_metadata(AlignedFileReader &reader, size_t &npts, size_t &ndim, size_t offset)
{
    ...
    if ((*(ctx.m_pRequestsStatus))[0] == IOContext::READ_SUCCESS)
    {
        npts = buf[0];
        ndim = buf[1];
        pageann::cout << "File has: " << npts << " points, " << ndim << " dimensions at offset: " << offset
                      << std::endl;
    }
    else
    {
        ...
        throw pageann::ANNException(str.str(), -1, __FUNCSIG__, __FILE__, __LINE__);
    }
}
                                 ...
                                 const size_t &rounded_dim, size_t offset)
{
    if (data == nullptr)
    {
        pageann::cerr << "Memory was not allocated for " << data << " before calling the load function. Exiting..."
                      << std::endl;
        throw pageann::ANNException("Null pointer passed to copy_aligned_data_from_file()", -1, __FUNCSIG__, __FILE__,
                                    __LINE__);
    }
    ...
    if (ndim != dim || npts != pts)
    {
        ...
        throw pageann::ANNException(ss.str(), -1, __FUNCSIG__, __FILE__, __LINE__);
    }
    ...
    while (i < npts)
    {
        ...
        for (int k = 0; k < read_requests.size(); k++)
        {
            ...
            {
                throw pageann::ANNException("Load data from file using AlignedReader failed.", -1, __FUNCSIG__,
                                            __FILE__, __LINE__);
            }
        }
    }
...
template <typename T> void read_array(AlignedFileReader &reader, T *data, size_t size, size_t offset)
{
    if (data == nullptr)
    {
        throw pageann::ANNException("read_array requires an allocated buffer.", -1);
    }
